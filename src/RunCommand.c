...
#ifdef ROSELAND_TCM2_COMPLIANCE_LOG
#include <stdio.h>
static void tcm2_interface_compliance_log(unsigned char *buf, uint32_t buf_len, const char* str)
{
    uint32_t i;
    printf("%s:\n", str);
    for (i = 1; i <= buf_len; i++) {
...
#endif // ROSELAND_TCM2_COMPLIANCE_LOG
		  ...
		  )
{
    setjmp(s_jumpBuffer);
#ifdef ROSELAND_TCM2_COMPLIANCE_LOG
    tcm2_interface_compliance_log(request, requestSize, "input");
#endif
    ExecuteCommand(requestSize, request, responseSize, response);
#ifdef ROSELAND_TCM2_COMPLIANCE_LOG
    tcm2_interface_compliance_log(*response, *responseSize, "output");
#endif
}
