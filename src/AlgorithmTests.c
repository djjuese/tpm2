...
#define HASH_CASE_FOR_TEST(HASH, hash)        case ALG_##HASH##_VALUE:	\
    ...
    if((testDigest->size != computed.t.size)
       || (memcmp(testDigest->buffer, computed.t.buffer, computed.b.size) != 0)) {
		   ROSELAND_TCM2_LOG("HASH(id:%x) test failed", hashAlg);
	SELF_TEST_FAILURE;
    }
    return TPM_RC_SUCCESS;
}
		       ...
		       )
{
    ...
    if(!MemoryEqual(encrypted, test->dataOut[mode - TPM_ALG_CTR],
		    test->dataInOutSize)) {
		ROSELAND_TCM2_LOG("symmetric(id:%x, mode:%x) encrypt test failed", test->alg, mode);
    ...
    if(!MemoryEqual(decrypted, test->dataIn, test->dataInOutSize)) {
		ROSELAND_TCM2_LOG("symmetric(id:%x, mode:%x) decrypt test failed", test->alg, mode);
	SELF_TEST_FAILURE;
    }
}
	 ...
	 )
{
	...
	// qsun - TCM2 need test SM2
#ifdef ROSELAND_TCM2  // qsun - TCM2 need SM2
	if (TEST_BIT(TPM_ALG_SM2, *toTest)) {
		LoadEccPoint(&Qe, &c_ecTestKey_sm2_QeX, &c_ecTestKey_sm2_QeY);
		if(TPM_RC_SUCCESS != CryptEccPointMultiply(&Z, TPM_ECC_SM2_P256, &Qe, &ds,
								NULL, NULL)) {
		SELF_TEST_FAILURE;
    	}
    	if(!MemoryEqual2B(&c_ecTestEcdh_sm2_X.b, &Z.x.b)
       	|| !MemoryEqual2B(&c_ecTestEcdh_sm2_Y.b, &Z.y.b)) {
			   ROSELAND_TCM2_LOG("SM2 Point Multiply test failed");
		SELF_TEST_FAILURE;
    	}
	} 
#endif // ROSELAND_TCM2
		     ...
		     )
{
	...
	TPM2B_DIGEST 			    *digest_in = (TPM2B_DIGEST*)&c_ecTestValue;
    testSig.sigAlg = scheme;
	...
	{
	  case TPM_ALG_ECDSA:
	  if(DEFAULT_TEST_HASH == ALG_SM3_256_VALUE)
	  	return TPM_RC_SUCCESS;
...
// qsun - TCM2 command TCM2_Sign has no parameters to pass userID, so the input is e, and hash not used internally.
		/* testSig.signature.sm2.hash = ALG_SM3_256_VALUE;
		eccScheme.details.anySig.hashAlg = ALG_SM3_256_VALUE;
		if(TEST_BIT(ALG_SM3_256_VALUE, g_toTest))
			TestHash(ALG_SM3_256_VALUE, toTest); */
	...
	if (scheme == TPM_ALG_SM2) {
    	LoadEccPoint(&testObject.publicArea.unique.ecc, &c_ecTestKey_sm2_QsX,
		 	&c_ecTestKey_sm2_QsY);
    	testObject.publicArea.parameters.eccDetail.curveID = TPM_ECC_SM2_P256;
		digest_in = (TPM2B_DIGEST*)&c_ecTest_sm2_Value;
	}
#endif // ROSELAND_TCM2

    if(TPM_RC_SUCCESS != CryptEccValidateSignature(&testSig, &testObject, digest_in)) {
	SELF_TEST_FAILURE;
	}
    CHECK_CANCELED;
    // Now sign and verify some data
    if(TPM_RC_SUCCESS != CryptEccSign(&testSig, &testObject,
				      digest_in, &eccScheme, NULL)) {
	SELF_TEST_FAILURE;
    }
    CHECK_CANCELED;
    if(TPM_RC_SUCCESS != CryptEccValidateSignature(&testSig, &testObject, digest_in)) {
	 ...
	 )
{
    ...
    UINT32                      counter = 0;
	if (KDF_TEST_ALG == TPM_ALG_SM3_256)
	return TPM_RC_SUCCESS;
    //
