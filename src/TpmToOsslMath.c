		  ...
		  )
{
    ...
    if (a->neg)
	printf("-");
    #ifdef WOLFSSL_BN_ULONG
    if (1) {
        mp_int* p = (mp_int*)a->internal;
        //int used, alloc, sign;
        mp_digit dp; 
        int j;
        notZero = 0;
        (void )d;
        //printf("(%0d/%0d) %llx ",p->used, p->alloc,p->dp[0]);
        for (i=p->used; i>0 ; i-- ) {
            dp = p->dp[i-1] ;
            //printf("%llx",dp);
            for (j=DIGIT_BIT;j>0;j-=4) {
                unsigned char pb = (dp>>(j-4)) & 0xF ;
                if (pb) notZero=1;
                if (notZero) printf("%01x", pb);
            }
        }
    }
    #else
	    ...
	    )
{
	...
	{
        #ifdef WOLFSSL_BN_ULONG
            //#warning("YOU SHOULD CHANGE HERE\n");
            VERIFY(((mp_int*)osslBn->internal)->used*DIGIT_BIT <= BnGetAllocated(bn) * RADIX_BITS);
            if (1) {
                BYTE *      buf = NULL;
                NUMBYTES    size = 0;
                size = BN_bn2bin(osslBn,NULL);
                if (size > 0)
                {
                    buf = malloc(size);
                    VERIFY(buf != NULL);
                    size = BN_bn2bin(osslBn, buf);
                    BnFromBytes(bn, buf, size);
                    free(buf);
                }
            }
        #else
	       ...
	       )
{
    ...
    if (1) {
        ...
        NUMBYTES    size = 0;
        //toInit->neg = 0;
        //toInit->internal = NULL;
        size = BnSizeInBits(initializer);
        size = (size+7)/8;
        buf = malloc(size);
        BnToBytes(initializer,buf,&size);
        if (0) { int i ; for (i=0;i<size;i++) printf("%02x",buf[i]); printf("\n");}
        //TODO: BN is allocated on stack.
        //toInit->internal = malloc(sizeof(mp_int));
        //mp_init((mp_int*)toInit->internal);
        BN_bin2bn(buf,size,toInit);
        BIGNUM_PRINT("Init:",toInit,1);
        free(buf);
    }
    #else
	  ...
	  )
{
    ...
    VERIFY(BN_div(dvTemp, bnResult, bnTemp, bnMod, CTX));
	     ...
	     )
{
    ...
    if (S == NULL)
    {
        S = (pointConst) & (AccessCurveData(E)->base);
        pS = EcPointInitialized(S, E);
    }
