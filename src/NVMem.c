...
#if FILE_BACKED_NV
#   include         <stdio.h>
static ROSELAND_FILE                *s_NvFile = NULL;		/* kgold made these static */
...
#   if defined _MSC_VER && 1
	...
	s_NvFile = NULL;
#   else
    s_NvFile = ROSELAND_OPEN(s_NvFilePath, mode);
#   endif
	     	...
	     	)
{
    ...
    ROSELAND_SEEK(s_NvFile, 0, SEEK_SET);
    OK = (NV_MEMORY_SIZE == ROSELAND_WRITE(s_NV, 1, NV_MEMORY_SIZE, s_NvFile));
    OK = OK && (0 == ROSELAND_FLUSH(s_NvFile));
    assert(OK);
    return OK;
}
/* C.6.2.3.	NvFileSize() */
	   ...
	   )
{
    ...
    long    fileSize;
    long    filePos = ROSELAND_TELL(s_NvFile);
    //
    assert(NULL != s_NvFile);

    int fseek_result = ROSELAND_SEEK(s_NvFile, 0, SEEK_END);
    NOT_REFERENCED(fseek_result); // Fix compiler warning for NDEBUG
    assert(fseek_result == 0);
    fileSize = ROSELAND_TELL(s_NvFile);
    assert(fileSize >= 0);
    switch(leaveAt)
	{
	  case SEEK_SET:
	    filePos = 0;
	  case SEEK_CUR:
	    irc = ROSELAND_SEEK(s_NvFile, filePos, SEEK_SET);
	    assert(irc == 0);
	    break;
	  case SEEK_END:
	    break;
		...
		)
{
    ...
    // If the file exists
    if(NvFileOpen("rw") >= 0) // peter: "r+b" ->"rw"
	{
	...
	{
	    printf("###qsun@_plat__NVEnable: new NVChip\n");
		 ...
		 )
{
    ...
    if(NULL != s_NvFile)
	{
	    ROSELAND_CLOSE(s_NvFile);    // Close NV file
	    ...
	    if(delete)
		{
		    ...
		    if(NvFileOpen("w") >= 0)
			{
			    ROSELAND_FLUSH(s_NvFile);
			    ROSELAND_CLOSE(s_NvFile);
			}
		}
	}
